...
2. **严禁碎句**：禁止生成少于{min_chars}个字的短句。即使原文有语气停顿，也不能单独成行。
   - 单字词（对、啊、哦、吗、嗯、哈哈 等）必须与前一句或后一句合并
   - 2-3字的短反应（如"真的吗"、"好"、"不对"等）必须与相邻句合并
   - 合并后每行至少{min_chars}个字
3. **合并策略**：
   - 先确保不碎句，再确保不超长
   - 同一说话人的连续短句必须合并
   - 不同说话人的简短交替（如"对" "好吗"）应合并为一句（如"对 好吗"）
4. 标点清理： 删除原文中所有的句号、逗号、感叹号、顿号。仅保留问号（？）。
5. 空格代标点： 删除标点后，所有原标点位置一律替换为一个半角空格。
6. 书名号保护： 书名号《》及其内部内容视为整体，禁止从中途断开。但如果书名号整体已超过 {max_length}，则必须拆分。
7. 强制断行： 即使原句没有任何标点，如果一行超过 {max_length} 个字符，也必须在语义合理处（主谓之间、动宾之间、转折处）强制断行。
8. 输出格式： 仅返回一个 JSON 字符串数组，每项为一行字幕。

⚠️ 严格执行：返回前逐行检查，确保 (1)无任何短句 (2)每行字符数 ≤ {max_length}。短句比超长更严重错误。

示例（max_length=10, min_chars=4）：
输入：哈哈看来你今天早上起来很不容易
输出：["哈哈 看来你今天早上", "起来很不容易"]

示例（max_length=12, min_chars=4）：
输入：大家好，我是小悦！欢迎来到今天的《大白话中文》！
输出：["大家好 我是小悦", "欢迎来到今天的", "《大白话中文》"]

示例（max_length=14, min_chars=4）：
输入：六点半，但是我每次都把它关掉，然后继续睡。
输出：["六点半 但是我每次", "都把它关掉 然后继续睡"]

示例（max_length=20, min_chars=4）：
输入：真的吗？但是我来中国以后，好像不管是身体哪儿不舒服，大家都会说多喝热水。
输出：["真的吗 但是我来中国以后", "好像不管是身体哪儿不舒服", "大家都会说多喝热水"]

示例（max_length=20, min_chars=4）：
输入：好 第一个场景 感冒了 怎么办 多喝热水 对 第二个场景 胃不舒服 怎么办 多喝热水
输出：["好 第一个场景 感冒了怎么办", "多喝热水 对 第二个场景", "胃不舒服怎么办 多喝热水"]
...
7. 强制断行： 即使原句没有任何标点，如果一行超过 {max_length} 个字符，也必须在语义合理处（主谓之间、动宾之间、转折处）强制断行。
8. 输出格式： 仅返回一个 JSON 字符串数组，每项为一行字幕。
9. 行号标记： 返回时用 [序号] 标注每句属于哪一行文本。**[序号] 不计入字数。**
...
输出：["[1]大家好 我是小悦", "[1]欢迎来到今天的", "[1]《大白话中文》", "[2]是啊 我每天早上都要", "[2]和我的闹钟 打架"]
...
示例（max_length=20, min_chars=4）：
输入：
[1]小悦: 真的吗？但是我来中国以后，好像不管是身体哪儿不舒服，大家都会说多喝热水。

输出：["[1]真的吗 但是我来中国以后", "[1]好像不管是身体哪儿不舒服", "[1]大家都会说多喝热水"]
